...
import { calculateTokenTax, formatPKR } from '../../utils/calculatorEngine';

const TokenTaxCalculator = () => {
  const [province, setProvince] = useState('punjab');
  const [cc, setCc] = useState(1300);
  const [invoiceValue, setInvoiceValue] = useState(4000000);
  const [regYear, setRegYear] = useState(new Date().getFullYear());
  const [filerStatus, setFilerStatus] = useState('filer');
  const [useEpay, setUseEpay] = useState(true);

  const res = calculateTokenTax({ province, cc, invoiceValue, regYear, filerStatus, useEpay });
  ...
  return (
    <div className="border-2 border-black dark:border-white bg-white dark:bg-zinc-950 p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
      <div className="mb-6 border-b-2 border-black dark:border-white pb-4">
        <span className="text-xs font-mono text-zinc-500">[ TOOL // 02 ]</span>
        <h2 className="text-2xl font-black tracking-tight uppercase mt-1">Annual Token Tax</h2>
      </div>
      ...
      <div className="flex flex-col gap-5 mb-8">
        ...
        {(province === 'punjab' || province === 'islamabad') && cc > 1000 && (
          <div className="flex flex-col gap-2">
            <label className="text-sm font-bold uppercase tracking-tight">Invoice Value (PKR)</label>
            <input 
              type="number" 
              value={invoiceValue} 
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm font-bold uppercase tracking-tight">Filer Status</label>
        ...
        {province === 'punjab' && (
          <label className="flex items-center gap-3 cursor-pointer">
            <input 
              type="checkbox" 
              checked={useEpay} 
              onChange={(e) => setUseEpay(e.target.checked)} 
              className="w-5 h-5 accent-red-600 border-2 border-black cursor-pointer"
            />
            <span className="text-sm font-bold uppercase tracking-tight">Apply ePay Punjab 5% Rebate</span>
          </label>
        )}
      </div>
      
      <div className="bg-zinc-100 dark:bg-zinc-900 border-2 border-black dark:border-white p-5">
        <h3 className="text-xs font-bold text-red-600 uppercase tracking-widest mb-1">Total Token Tax</h3>
        ...
        <div className="space-y-2 border-t-2 border-black dark:border-white pt-4 mt-2">
          <div className="flex justify-between items-center text-sm font-bold">
            <span className="text-zinc-500 uppercase">Provincial Tax</span>
            <span>{formatPKR(res.baseTax)}</span>
          </div>
          <div className="flex justify-between items-center text-sm font-bold">
            <span className="text-zinc-500 uppercase">Sec 234 WHT</span>
            <span>{formatPKR(res.wht)}</span>
          </div>
          {res.age >= 10 && (
            <div className="text-xs font-bold text-red-600 mt-2">*Exempt from WHT (Over 10 yrs)</div>
          )}
        </div>
      </div>
    </div>
...
export default TokenTaxCalculator;
