...
export default function SaveCarButton({ listingId, platform, title, savedListingIds = new Set(), onUnsave }) {
  ...
  return (
    <button
      ...
      className="glass-thin p-2.5 hover:border-white/20 hover:scale-105 transition-all disabled:opacity-50"
    >
      {isLoading ? (
        ...
        >
          <path
            ...
            fill={isSaved ? "var(--df-accent)" : "none"}
            stroke={isSaved ? "var(--df-accent)" : "var(--df-text)"}
            strokeWidth="2"
          />
        </svg>
      )}
