...
export default function SaveCarButton({ listingId, platform, title, savedListingIds = new Set(), onUnsave }) {
  ...
  useEffect(() => {
    // Check initial state from the parent's provided set of saved IDs
  ...
  return (
    <button
      ...
      className="p-2 rounded-full bg-white/80 backdrop-blur-sm shadow-sm hover:bg-white transition-colors disabled:opacity-50"
    >
      {isLoading ? (
